...
import { exportInvoiceToPDF } from "@/features/invoices/lib/exportToPDF";
...
export default function CustomerInvoices({ customerId }: CustomerInvoicesProps) {
  ...
  const [customerDetails, setCustomerDetails] = useState<Tables<'customers'> | null>(null);
  ...
  const fetchGroupedInvoices = useCallback(async () => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
  ...
  }, [fetchGroupedInvoices, fetchCustomerDetails]);
