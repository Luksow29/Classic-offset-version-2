...
const CustomerOrderRequestForm: React.FC<CustomerOrderRequestProps> = ({ customer, reorderData, setReorderData }) => {
    ...
    const onSubmit = async (values: OrderRequestFormValues) => {
        // **SOLUTION:** Explicitly include all fields including calculated ones
        const requestDataWithCustomerInfo = {
            orderType: values.orderType,
            productId: values.productId,
            quantity: values.quantity,
            rate: values.rate || 0,  // Explicitly include rate
            subtotal: values.subtotal || 0,  // Explicitly include subtotal
            totalAmount: values.totalAmount || 0,  // Explicitly include totalAmount
            deliveryDate: values.deliveryDate ? values.deliveryDate.toISOString() : null,
            designNeeded: values.designNeeded,
            notes: values.notes || '',
